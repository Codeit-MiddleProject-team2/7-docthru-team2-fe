...
import { challenges as allChallenges } from "@/mock/mockChallenges";
...
// API 시뮬
const mockFetchChallenges = async (tab, page = 1, query = "", sort = "") => {
  console.log(
    `Fetching from mock file: tab=${tab}, page=${page}, query=${query}, sort=${sort}`
  );

  return new Promise((resolve) => {
    setTimeout(() => {
      let results = [...allChallenges];
      results = results.filter(challenge => challenge.status === tab);
      if (query) {
      ...
      switch (sort) {
        case "마감일 가까운 순":
          ...
          break;
        case "최신 참여 순":
          results.sort((a, b) => new Date(b.participatedAt) - new Date(a.participatedAt));
          break;
        case "최신순":
        default:
...
export default function MyChallengesCompletedPage() {
//   const sample = {
//     id: 2,
...
//   };
  ...
  const loadChallenges = async (isNewSearch = false) => {
    ...
    const newChallenges = await mockFetchChallenges(
      activeTab, currentPage, searchQuery, sortOption
    );

    if (newChallenges.length < ITEMS_PER_PAGE) {
      setHasMore(false);
    }
